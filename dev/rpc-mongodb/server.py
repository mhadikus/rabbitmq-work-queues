...
def main():
    ...
    channel.queue_declare(queue='my-rpc-mongo-queue', durable=True)

    # Don't dispatch a new request to this server until it has processed and acknowledged the previous one
    channel.basic_qos(prefetch_count=1)

    channel.basic_consume(queue='my-rpc-mongo-queue', on_message_callback=on_request_received)
...
def on_request_received(ch, method, properties, body):
    ...
    try:
        ...
        bulk_write_result = db.my_collection.bulk_write(requests)
